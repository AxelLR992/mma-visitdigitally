        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                ...
                <div class="container-fluid">
                    ...
                    $stmt = $mysqli->prepare("SELECT recordid, category, parent_cat_id, cat_level, img_sm, img_bg, description, status, display_priority FROM categories WHERE recordid=?") or die(mysqli_error($mysqli));
                    $stmt->bind_param('i', $_GET['rid']);
                    $stmt->execute();    // Execute the prepared query.
                    $stmt->store_result();
                    // get variables from result.
                    $stmt->bind_result($db_recordid, $db_category, $db_parent_cat_id, $db_cat_level, $db_img_sm, $db_img_bg, $db_description, $db_status, $db_display_priority);
                    $stmt->fetch();
                    ?>
                    <div class="row">
                        <form action="process_edit.php" method="post" data-toggle="validator" role="form">
                            ...
                            <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12" style="padding-left: 10px; padding-right: 10px; padding-bottom: 10px;">                                
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="category" name="category" value="<?php echo $db_category ?>">
                                </div>
                                 <div class="form-group">
                                    <label for="parent_cat_id" class="control-label">Parent Category</label>
                                    <select name="parent_cat_id" class="input-text">
                                                <option value="0" <?php if ($db_parent_cat_id == 0) {echo 'selected';} ?>>N/A</option>                                        
                                        <?php
// Query list records
                                        $mysqli->query("SET NAMES 'utf8'");
                                        ...
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ?>                                        
                                                <option value="<?php echo $row["recordid"]; ?>" <?php if ($db_parent_cat_id == $row["recordid"]) {echo 'selected';} ?>><?php echo $row["category"]; ?></option>
                                                <?php
                                            }
                                        }
                                        $result->close;
                                ...
                                <div class="form-group">
                                    <label for="cat_level" class="control-label">Category Level</label>
                                    <select name="cat_level" class="input-text">
                                        <option value="1" <?php if ($db_cat_level == '1') {echo 'selected';} ?>>1</option>
                                        <option value="2" <?php if ($db_cat_level == '2') {echo 'selected';} ?>>2</option>
                                        <option value="3" <?php if ($db_cat_level == '3') {echo 'selected';} ?>>3</option>                                       
                                    </select>                                    
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" class="input-text" rows="3"><?php echo $db_description ?>
                                    </textarea>
                                </div>
                                 <div class="form-group">
