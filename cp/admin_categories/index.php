...
// Count table records
                                ...
                                <div class="panel-heading">
                                    <div class="row">
                                        ...
                                        <div class="col-xs-9 text-right">
                                            <div><b><?php echo $rc ?></b> Categories</div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <table id="list_categories" class="display" width="98%">
                                <thead>
                                    <tr>
                                        <th>recordid</th>
                                        <th>category</th>
...
// Query list records
                                    ...
                                    $sql = "SELECT recordid, category, cat_level, parent_cat_id, status, display_priority FROM categories order by category";
                                    $result = $mysqli->query($sql);
                                    $i = 0;
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            if ($row["parent_cat_id"] <> 0) {
                                                $sql2 = "SELECT category FROM categories where recordid =" . $row['parent_cat_id'] ;
                                                $result2 = $mysqli->query($sql2);
                                                $row2 = $result2->fetch_assoc();
                                                $parent_cat_name = $row2['category'];
                                                $result2->close;
                                                $row2->close;
                                            }
                                            ?>                                
